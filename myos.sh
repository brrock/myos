...
kernel="https://cdn.kernel.org/pub/linux/kernel/v6.x/linux-6.10.7.tar.xz"
# ---------------------------------------
...
if [ $answer != "y" ] ; then
  ...
  git clone https://git.busybox.net/busybox.git busybox
  cd busybox
  # BusyBox configuration --------------------------------
  make defconfig
  # ------------------------------------------------------
  # patches busybox
  FILE_PATH="networking/tc.c"
  BACKUP_PATH="networking/tc.c.bak"
  source_c_file="patches/busybox/networking/tc.c"
  tc_c_content=$(cat "$source_c_file")
  ...
  # Apply the changes to networking/tc.c
  cat << 'EOF' > "$FILE_PATH"
  $tc_c_content
EOF
  make
